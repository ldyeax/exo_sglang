...
def _v4_triton_decode_dispatch(
    ...
    **_unused: Any,
    ...
    # Both portable kernels load the complete disjoint (token, head) query
    # vector into registers before their attention loop and perform their only
    # output store after the query's last use. Production Q is already BF16,
    # so reuse that storage for the result. At a 4K Pro prefill chunk this
    # removes a second 512 MiB [tokens, heads, head_dim] allocation.
    out = (
        q_2d
        if q_2d.dtype == torch.bfloat16
        else torch.empty(
