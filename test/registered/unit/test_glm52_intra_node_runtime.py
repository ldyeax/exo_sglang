...
def test_launch_plan_is_executable_native_pd_with_private_process_state(
    ...
    assert _argv_value(plan.prefill.argv, "--cuda-graph-backend-decode") == "disabled"
...
def test_worker_commands_parse_with_current_server_args(tmp_path: Path) -> None:
    plan = build_glm52_pd_launch_plan(
        _config(tmp_path),
        observations=_observations(),
        host_capacity_bytes=128 * _GIB,
    ...
    for command in (plan.prefill, plan.decode):
        parser = argparse.ArgumentParser()
        ServerArgs.add_cli_args(parser)
        parsed = parser.parse_args(command.argv[3:])
        assert parsed.tp_size == 1
        assert parsed.pp_size == 1
        assert parsed.kt_stream_prefill
